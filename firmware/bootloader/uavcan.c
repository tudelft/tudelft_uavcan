...
static THD_WORKING_AREA(can1_uavcan_wa, 256);
...
static THD_WORKING_AREA(can2_rx_wa, 256);
static THD_WORKING_AREA(can2_tx_wa, 256);
static THD_WORKING_AREA(can2_uavcan_wa, 256);
...
static void uavcanInitIface(struct uavcan_iface_t *iface) {
  ...
  chThdCreateStatic(iface->thread_rx_wa, iface->thread_rx_wa_size, NORMALPRIO + 7, can_rx, (void*)iface);
